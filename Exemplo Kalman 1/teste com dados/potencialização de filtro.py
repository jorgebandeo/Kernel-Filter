...
def plot(original, filtered, nmse_values):
    ...
    plt.tight_layout()
    plt.savefig('Exemplo Kalman 1/teste com dados/teste potencializado csv.png')
    plt.show()
...
def kfilter(rw, numStates, seq, iterations=1):
    ...
    for iteration in range(iterations):
        if iteration > 0:
            # Usa o resultado filtrado da iteração anterior
            rw = filtered.copy()
    ...
    filtered = np.clip(filtered, np.min(rw), np.max(rw))  # Exemplo de pós-processamento
...
# Ajustar tamanho de nmse_values para plotagem
nmse_values = nmse_values[:len(filtered_signal) - numStates]

# Plotar os resultados
plot(noisy_signal[numStates:], filtered_signal[numStates:], nmse_values)
